...
def generalized_cross_entropy(logits, one_hot_labels,q=0.6):
    ...
    if q == 0.0:
        ...
        loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(one_hot_labels),logits=logits)
        loss = tf.reduce_mean(input_tensor=loss)
    else:
        exp_logits = tf.exp(logits)
        normalizer = tf.reduce_sum(input_tensor=exp_logits,axis=-1,keepdims=True)
        normalizer_q = tf.pow(normalizer,q)
        exp_logits_q = tf.exp(logits*q)
        f_j_q = exp_logits_q / normalizer_q
        loss = (1 - f_j_q)/q
        loss = tf.reduce_sum(input_tensor=loss * one_hot_labels, axis=-1)
        loss = tf.reduce_mean(input_tensor=loss)
    return loss
