...
class HighLevelSupervisionNetwork:
    ...
    def __init__(self, num_features, num_classes, num_rules,
        ...
        self.rule_classes = tf.convert_to_tensor(value=rule_classes)
        ...
        self.dropout_keep_prob = tf.compat.v1.placeholder(tf.float32,name="keep_prob")
        ...
        self.writer = tf.compat.v1.summary.FileWriter(config.tensorboard_dir, self.sess.graph)
        # Now that all variables and the session is created, create a
        # checkpoint saver. We use a single saver for all variables
        self.mru_saver = MRUCheckpoint(config.checkpoint_dir, self.sess, tf.compat.v1.global_variables())
    ...
    def make_f_d_train_ops(self):
        ...
        inc_f_d_global_step = tf.compat.v1.assign_add(self.f_d_global_step, 1)
        self.global_steps[f_d] = self.f_d_global_step
        self.f_d_adam_lr = tf.compat.v1.placeholder(tf.float32,name='f_d_U_adam_lr')

        # [batch size, features]
        self.f_x = tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_features],
                name='f_d_x')
        self.f_d_labels = tf.compat.v1.placeholder(tf.float32, shape=[None,
        ...
        self.f_d_preds = tf.argmax(input=self.f_d_probs, axis=-1)        

        
        model_loss = tf.nn.softmax_cross_entropy_with_logits(
            labels=tf.stop_gradient(self.f_d_labels), logits=self.f_d_logits)

        self.f_d_loss = tf.reduce_mean(input_tensor=model_loss)

        self.f_d_optimizer = tf.compat.v1.train.AdamOptimizer(
    ...
    def make_f_d_U_train_ops(self):
        ...
        inc_f_d_U_global_step = tf.compat.v1.assign_add(self.f_d_U_global_step, 1)
        ...
        * r : A binary matrix of shape [batch_size, num_rules]
        ...
        '''
        self.f_d_U_adam_lr = tf.compat.v1.placeholder(tf.float32,name='f_d_U_adam_lr')
        self.f_d_U_x = tf.compat.v1.placeholder(
                tf.float32,
                shape=[None, self.num_features],
                name='f_d_U_x')
        # l
        self.f_d_U_l = tf.compat.v1.placeholder(
                tf.int32,
                shape=[None, self.num_rules],
                name='f_d_U_l')
        # m 
        self.f_d_U_m = tf.compat.v1.placeholder(tf.float32, shape=[None,
            self.num_rules], name='f_d_U_m')
        # L
        L = self.f_d_U_L = tf.compat.v1.placeholder(tf.int32, shape=[None, 1], name='f_d_U_L')
        # d
        d = self.f_d_U_d = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name='f_d_U_d')
        L = tf.squeeze(L)
        d = tf.squeeze(d)

        r = self.f_d_U_r = tf.compat.v1.placeholder(tf.float32, shape=[None,self.num_rules], name='f_d_U_r')
        ...
        self.f_d_U_num_d = tf.reduce_sum(input_tensor=d) #number of labeled instances in a batch
        ...
        self.f_d_U_preds = tf.argmax(input=self.f_d_U_probs, axis=-1)
        ...
        LL_theta = tf.nn.softmax_cross_entropy_with_logits(logits=f_logits,
                labels=tf.stop_gradient(L_one_hot))
        LL_theta = d * LL_theta
        LL_theta = tf.reduce_mean(input_tensor=LL_theta) # loss of f network on labeled data d
        ...
        f_cross_training_optimizer = tf.compat.v1.train.AdamOptimizer(
                learning_rate=self.f_d_U_adam_lr,
                name='f_d_U_Adam')

        training_var_list = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES)
        ...
        if 'gcross' == self.config.mode:
            self.f_d_U_snork_L = tf.compat.v1.placeholder(
                    tf.float32,
        ...
        if 'label_snorkel' == self.config.mode or 'pure_snorkel' == self.config.mode:
            ...
            self.pure_snorkel_loss = tf.nn.softmax_cross_entropy_with_logits(
                                     labels=tf.stop_gradient(self.f_d_U_snork_L),logits=f_logits)
            self.pure_snorkel_loss = tf.reduce_mean(input_tensor=self.pure_snorkel_loss)
        ...
        if 'learn2reweight' == self.config.mode:
            ...
            raw_d_x = tf.compat.v1.get_variable(name="raw_d_x", initializer=self.raw_d_x, trainable=False)
            raw_d_x = tf.cast(raw_d_x, dtype=tf.float32)
            raw_d_L = tf.compat.v1.get_variable(name="raw_d_L", initializer=self.raw_d_L, trainable=False)
            raw_d_L = tf.cast(raw_d_L, dtype=tf.int32)
            ...
            # [batch_size]
            epsilon = tf.zeros(tf.shape(input=self.f_d_U_x)[0])
            ...
            f_logits = self.f_network(f_dict, self.num_classes, reuse=True,
                dropout_keep_prob=self.dropout_keep_prob)
            #[batch_size]
            unweighted_loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(self.f_d_U_snork_L),logits=f_logits)
            weighted_loss = epsilon * unweighted_loss 
            weighted_loss = tf.reduce_sum(input_tensor=weighted_loss)
            
            # 3. compute grads of ewl wrt thetas
            thetas = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=self.f_var_scope)
            ...
            unweighted_loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(raw_d_L_one_hot),
                                                                          logits=f_logits_on_d)
            unweighted_loss = tf.reduce_mean(input_tensor=unweighted_loss)
            ...
            w_norm = w_tilde/(tf.reduce_sum(input_tensor=w_tilde) + 1e-25)
    ...
    def compute_LL_phi(self, w_logits, weights, l, m, L, d, r):
        ...
        loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(tf.equal(l,L), dtype=tf.float32),
                                                       logits=w_logits)
        loss = m*loss
        loss = (tf.cast(tf.not_equal(l,L), dtype=tf.float32) * loss) + (tf.cast(tf.equal(l,L), dtype=tf.float32) * r * loss)
        gcross_loss = gcross_utils.generalized_cross_entropy_bernoulli(weights,0.2)
        gcross_loss = gcross_loss * m * tf.cast(tf.equal(l,L), dtype=tf.float32) * (1-r)
    ...
    def implication_loss(self, weights, f_probs, m, rule_classes, num_classes, d):
        ...
        obj = m*tf.math.log(obj + psi)
        obj = tf.reduce_sum(input_tensor=obj, axis=-1)
        obj = obj * (1-d) #defined only for instances in U, so mask by (1-d)
        obj = tf.reduce_mean(input_tensor=obj)
        return -obj
    ...
    def get_weights_and_logits_f_d_U(self, x):
        ...
        mul = tf.convert_to_tensor(value=[1, self.num_rules_to_train])
        ...
        rules_int = tf.convert_to_tensor(value=[list(range(0,
        ...
        tilevar = tf.convert_to_tensor(value=[batch_size, 1])
    ...
    def joint_scores_from_f_and_w(self,weights,m,f_probs):
        ...
        weights_mask = tf.cast(tf.greater(weights,0.5), dtype=tf.float32)
        #[batch_size, num_rules, 1]
        ...
        sum_rule_firings = tf.reduce_sum(input_tensor=m,axis=1)
        result = m*rule_weight_product #+ (1-m)
        #[batch_size, num_classes]
        result = tf.reduce_sum(input_tensor=result,axis=1)/(sum_rule_firings+1e-20)
