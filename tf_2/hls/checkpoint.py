...
class MRUCheckpoint():
    def __init__(self, path, session, variables):
        ...
        self.sess = session
        # max_to_keep
        self.saver = tf.compat.v1.train.Saver(variables, max_to_keep=1)
...
def test_mru_checkpoints(num_to_keep):
    global_step = tf.compat.v1.get_variable(name='mru_global_step_%d' % num_to_keep, initializer=10, dtype=tf.int32)
    inc = tf.compat.v1.assign_add(global_step, 1)
    ...
    ckpt = MRUCheckpoint(ckpt_path, sess, tf.compat.v1.global_variables())
    ...
    assgn_op = tf.compat.v1.assign(global_step, 5)
    sess.run(assgn_op)
...
def test_checkpoint():
    v = tf.compat.v1.get_variable(name='v', initializer=12, dtype=tf.int32)
    v1 = tf.compat.v1.assign_add(v, 1)
    sess = tf.compat.v1.Session()
    sess.run(tf.compat.v1.global_variables_initializer())
    assert sess.run(v) == 12
    sess.run(v1)
    assert sess.run(v) == 13
    ckpt = MRUCheckpoint('/tmp/checkpoints', sess, tf.compat.v1.global_variables())
    ckpt.save()

    sess1 = tf.compat.v1.Session()
    sess1.run(tf.compat.v1.global_variables_initializer())
    assert sess1.run(v) == 12
    ckpt1 = MRUCheckpoint('/tmp/checkpoints', sess1, tf.compat.v1.global_variables())
    ckpt1.restore()
    assert sess1.run(v) == 13
    sess1.run(v1)
    assert sess1.run(v) == 14

    ckpt2 = MRUCheckpoint('/tmp/bad-ckpt-path', sess1, tf.compat.v1.global_variables())
...
class BestCheckpoint():
    def __init__(self, path, prefix, session, num_checkpoints, variables, global_step):
        ...
        self.saver = tf.compat.v1.train.Saver(variables, max_to_keep=None, save_relative_paths=True) 
...
def test_best_ckpt():
    global_step = tf.compat.v1.get_variable(name='global_step', initializer=50, dtype=tf.int32)
    ...
    best1 = BestCheckpoint(best_checkpoint_dir, 'foo-bar', sess1, 3, tf.compat.v1.trainable_variables(), global_step)
    best2 = BestCheckpoint(best_checkpoint_dir, 'foo-bar', sess2, 3, tf.compat.v1.trainable_variables(), global_step)
...
def test_checkmate():
    global_step = tf.compat.v1.get_variable(name='checkmate_global_step', initializer=12, dtype=tf.int32) 
    inc_global_step_op = tf.compat.v1.assign_add(global_step, 1)
    sess = tf.compat.v1.Session()
    sess1 = tf.compat.v1.Session()
    
    sess.run(tf.compat.v1.global_variables_initializer())
    saver = tf.compat.v1.train.Saver(tf.compat.v1.trainable_variables(), max_to_keep=None)
    best_checkpoint_dir = '/tmp/best_ckpt_%.6f' % np.random.rand()
...
class CheckpointsFactory:
    ...
    def init_saver(self, sess, mode, global_steps):
        ...
        self.best_savers[mode] = BestCheckpoint(ckpt_dir, mode, sess,
                config.num_checkpoints, tf.compat.v1.global_variables(), global_steps[mode])
