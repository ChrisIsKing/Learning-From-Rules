...
def w_network_fully_connected(w_var_scope, num_rules, w_dict, reuse=False, dropout_keep_prob=1.0):
    ...
    with tf.compat.v1.variable_scope(w_var_scope, reuse=reuse, initializer=create_initializer()) as vs:
        for i, num_neurons in enumerate(config.w_layers):
            cur_layer = tf.compat.v1.layers.dense(prev_layer, num_neurons,
                    activation=tf.nn.relu, name='w_layer_%d' % i)
            if config.network_dropout is True:
                cur_layer = tf.nn.dropout(cur_layer, rate=1 - (dropout_keep_prob))
            prev_layer = cur_layer
        logit = tf.compat.v1.layers.dense(prev_layer, 1, name='w_linear_layer')
    return logit
...
def f_network_fully_connected(f_var_scope, f_dict, num_classes, 
    ...
    x = f_dict['x']
    if not ph_vars:
        with tf.compat.v1.variable_scope(f_var_scope, reuse=reuse, initializer=create_initializer()) as vs:
            prev_layer = x
            ...
            logits = tf.compat.v1.layers.dense(prev_layer,num_classes)
    else:
