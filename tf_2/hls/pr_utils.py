...
def exp_term_for_constraints(rule_classes, num_classes, C):
	rule_classes_tensor = tf.cast(tf.convert_to_tensor(value=rule_classes), dtype=tf.float32)
	...
	class_types_tensor = tf.cast(tf.convert_to_tensor(value=np.arange(num_classes).reshape(num_classes,1)), dtype=tf.float32)	
	#[num_classes,num_rules]
	class_rule_constraint = tf.cast(tf.equal(class_types_tensor,rule_classes_tensor), dtype=tf.float32) - 1.0
...
def pr_product_term(weights, rule_classes, num_classes, C):
	...
	product_term = tf.reduce_prod(input_tensor=t,axis=-1)
	#[batch_size, num_classes]
	product_term = tf.transpose(a=product_term)
	return product_term
...
def get_q_y_from_p(f_probs, weights, rule_classes, num_classes, C):
	...
	normalizer = tf.reduce_sum(input_tensor=result,axis=-1,keepdims=True)
...
def get_q_r_from_p(f_probs, weights, rule_classes, num_classes, C):
	...
	class_rule_constraint = tf.transpose(a=exp_term_for_constraints(rule_classes, num_classes, C))
	...
	sum_over_y_term = tf.reduce_sum(input_tensor=product_term,axis=-1)
	prob_q_r_eq_1 = weights * sum_over_y_term


	prob_q_r_eq_0 = tf.reduce_sum(input_tensor=f_probs * product_term, axis=-1)
...
def theta_term_in_pr_loss(f_logits, f_probs, weights, rule_classes, num_classes, C, d):
	...
	cross_ent_q_p = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(q_y),logits=f_logits)
	cross_ent_q_p = (1-d) * cross_ent_q_p #defined only for instances in U, so mask by (1-d)
	result = tf.reduce_mean(input_tensor=cross_ent_q_p)
	return result
...
def phi_term_in_pr_loss(m, w_logits, f_probs, weights, rule_classes, num_classes, C, d):
	...
	cross_ent_q_w = tf.reduce_sum(input_tensor=cross_ent_q_w*m,axis=-1)
	...
	cross_ent_q_w = tf.reduce_mean(input_tensor=cross_ent_q_w)
