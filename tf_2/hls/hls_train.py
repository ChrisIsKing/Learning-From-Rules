...
class HLSTrain():
    ...
    def make_f_summary_ops(self):
        with tf.compat.v1.name_scope('f_summaries'):
            self.f_d_loss_ph = tf.compat.v1.placeholder(tf.float32, shape=None, name='f_d_loss_placeholder')
            self.f_d_loss = tf.compat.v1.summary.scalar('f_d_loss', self.f_d_loss_ph)

            self.f_d_f1_score_ph = tf.compat.v1.placeholder(tf.float32, shape=None, name='f_d_f1_score_placeholder')
            self.f_d_f1_score = tf.compat.v1.summary.scalar('f_d_f1_score_1', self.f_d_f1_score_ph)

            self.f_d_accuracy_ph = tf.compat.v1.placeholder(tf.float32, shape=None, name='f_d_accuracy_placeholder')
            self.f_d_accuracy = tf.compat.v1.summary.scalar('f_d_accuracy', self.f_d_accuracy_ph)

            self.f_d_avg_f1_score_ph = tf.compat.v1.placeholder(tf.float32, shape=None, name='f_d_avg_f1_score_placeholder')
            self.f_d_avg_f1_score = tf.compat.v1.summary.scalar('f_d_avg_f1_score', self.f_d_f1_score_ph)

            self.f_d_summaries = tf.compat.v1.summary.merge([self.f_d_loss, self.f_d_f1_score,
